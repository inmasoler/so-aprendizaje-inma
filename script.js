...
function renderGlossary() {
  ...
  const catText   = {rendering:'#90c8ff',arquitectura:'#a8f0c8',datos:'#f5a97f',deploy:'#d8a8f5',ds:'#90c8ff'};
...
const DNS_CONCEPTS = [
  { name:'Nameserver (NS)', type:'Concepto DNS', color:'#4dd9e0', def:'El servidor que almacena y sirve la tabla de registros DNS de tu dominio. Cuando compras un dominio, puedes apuntar sus NS a los de tu hosting (Verpex, Cloudflare, etc.) para que ese servidor controle todos tus registros.', example:'Compras el dominio en Namecheap → cambias los NS a ns1.verpex.com → ahora cPanel de Verpex gestiona el DNS.', warn:'' },
  { name:'Zona DNS', type:'Concepto DNS', color:'#4dd9e0', def:'El archivo completo de configuración de todos los registros DNS de un dominio. En cPanel puedes editarla directamente desde "Zone Editor" o "DNS Zone". Un error aquí puede tumbar tu web o email.', example:'La zona DNS de miweb.com contiene sus registros A, MX, TXT, CNAME y NS en un único archivo de configuración.', warn:'' },
  { name:'TTL (Time To Live)', type:'Concepto DNS', color:'#4dd9e0', def:'Tiempo en segundos que los resolvers DNS cachean un registro antes de volver a consultarlo. TTL bajo (300s) = cambios rápidos pero más consultas. TTL alto (86400s = 24h) = cambios lentos pero más eficiente.', example:'Antes de migrar un servidor, baja el TTL a 300s. Así si hay problemas, la reversión se propaga en minutos.', warn:'' },
  { name:'Propagación DNS', type:'Concepto DNS', color:'#4dd9e0', def:'El tiempo que tarda un cambio de DNS en llegar a todos los resolvers del mundo. Puede tardar desde minutos hasta 48h dependiendo del TTL anterior y los caches de los ISPs.', example:'Cambias el registro A de tu dominio. En España ya apunta al nuevo servidor pero en EEUU aún va al antiguo. Esto es propagación en curso.', warn:'' },
];

const CPANEL_CONCEPTS = [
  { name:'Virtual Host', type:'Servidor web', color:'#c9a7f5', def:'Configuración que permite a un mismo servidor físico responder a múltiples dominios. cPanel crea automáticamente un virtual host por cada dominio/subdominio que añades, configurando desde dónde sirve los archivos (public_html) y cómo los procesa.', example:'En un servidor con cPanel puedes tener miweb.com → /home/user/public_html y tienda.miweb.com → /home/user/tienda/public_html', warn:'' },
  { name:'public_html', type:'Estructura archivos', color:'#c9a7f5', def:'La carpeta raíz pública de tu dominio en cPanel. Todo lo que pones aquí es accesible desde el browser. Es el equivalente al /dist o /out de tu build de Astro/Next.js que subas por FTP o deploy.', example:'Subes index.html a public_html → el usuario accede a miweb.com y ve ese archivo.', warn:'Nunca pongas archivos con credenciales (.env) dentro de public_html — serían accesibles desde internet.' },
  { name:'PHP vs HTML en servidor', type:'Procesamiento', color:'#c9a7f5', def:'cPanel sirve archivos HTML directamente (el servidor los lee y los envía). PHP se procesa primero: el servidor ejecuta el código PHP, genera HTML y lo envía. Por eso PHP puede leer bases de datos y HTML no.', example:'WordPress es PHP: el servidor ejecuta WordPress, que consulta MySQL y genera el HTML de cada página antes de enviarlo.', warn:'' },
  { name:'phpMyAdmin', type:'Base de datos', color:'#c9a7f5', def:'Interfaz web para gestionar bases de datos MySQL/MariaDB incluida en cPanel. Permite crear bases de datos, tablas, ejecutar queries SQL, importar/exportar datos y ver la estructura sin usar línea de comandos.', example:'WordPress necesita una base de datos MySQL. En cPanel: crear DB → crear usuario → asignar permisos → conectar WordPress con las credenciales.', warn:'' },
  { name:'JetBackup 5', type:'Backup y recuperación', color:'#c9a7f5', def:'Plugin de backup para cPanel que permite hacer y restaurar copias de seguridad completas (archivos, bases de datos, emails, configuraciones). Permite restaurar desde puntos específicos sin perder todo.', example:'Tu WordPress se rompe tras una actualización. Con JetBackup restauras el backup de ayer en 2 minutos.', warn:'' },
  { name:'SSH (Secure Shell)', type:'Acceso y seguridad', color:'#c9a7f5', def:'Protocolo de acceso remoto seguro al servidor mediante línea de comandos. Cifrado extremo-a-extremo. Más potente que FTP: puedes ejecutar cualquier comando, instalar software, reiniciar servicios.', example:'ssh usuario@85.43.22.10 → accedes al servidor y puedes ejecutar git pull, npm install, reiniciar Nginx, etc.', warn:'Desactiva el acceso root por SSH. Usa claves SSH en lugar de contraseñas.' },
  { name:'SSL/TLS con Let\'s Encrypt', type:'Seguridad', color:'#c9a7f5', def:'cPanel incluye AutoSSL para instalar certificados SSL gratuitos de Let\'s Encrypt automáticamente. SSL cifra la comunicación entre el browser y el servidor (el candado verde). Sin SSL, los datos viajan en texto plano.', example:'cPanel instala automáticamente un certificado para miweb.com y www.miweb.com. El browser muestra el candado.', warn:'Let\'s Encrypt caduca cada 90 días. cPanel lo renueva automáticamente, pero vigila que no falle.' },
  { name:'Verpex', type:'Hosting', color:'#c9a7f5', def:'Hosting que habilita y gestiona cPanel para sus clientes. Con Verpex tienes acceso a un cPanel completo donde puedes crear múltiples dominios, bases de datos, emails y configurar todo el servidor desde la interfaz visual.', example:'Con tu cuenta Verpex accedes a cPanel → Domains → Add Domain → introduces tu dominio → cPanel crea la carpeta, el virtual host y la zona DNS.', warn:'' },
];

const CACHE_TYPES = [
  { name:'Caché estática', type:'Archivos JS/CSS/imágenes', color:'#a8f0c8', desc:'El servidor guarda en memoria o disco los archivos estáticos (JS, CSS, imágenes) y los sirve sin leerlos de disco cada vez. Headers Cache-Control y ETag controlan cuánto tiempo los guarda el browser.', pros:'Trivial de implementar. Nginx/Apache lo hacen solos.', cons:'Si actualizas un archivo, el browser puede seguir usando la versión vieja hasta que expire el caché.' },
  { name:'Caché de objeto (Redis)', type:'Datos de base de datos', color:'#7eb8f7', desc:'Redis guarda en memoria los resultados de consultas a la base de datos. Próximas peticiones idénticas se responden desde RAM sin tocar MySQL. En WordPress lo gestiona el plugin LiteSpeed Cache u Object Cache Pro.', pros:'Drástica reducción de queries a DB. Ideal para sites con mucho tráfico.', cons:'Requiere configurar Redis (host, puerto, contraseña, memoria máxima). Si el caché y la DB desincroniza hay problemas.' },
  ...
  { name:'CDN (Content Delivery Network)', type:'Red distribuida global', color:'#c9a7f5', desc:'Tu web se replica en servidores de todo el mundo. El visitante recibe los datos del nodo más cercano a su ubicación. Cloudflare es el CDN más usado — también actúa como proxy de seguridad.', pros:'Latencia mínima para usuarios globales. Protección DDoS incluida en Cloudflare.', cons:'Cambios pueden tardar en propagarse (purgar caché del CDN). Más complejo de configurar correctamente.' },
];

const CACHE_CONCEPTS = [
  { name:'Redis', type:'Motor de caché', color:'#7eb8f7', def:'Base de datos en memoria ultrarrápida usada principalmente como caché. Guarda pares clave-valor en RAM. En un servidor cPanel tienes que activar el módulo PHP de Redis, conocer host/puerto/contraseña y configurarlo en WordPress con un plugin como LiteSpeed Cache.', example:'WordPress hace 80 queries a MySQL por pageview. Con Redis Object Cache, las queries se cachean: segunda visita → 3 queries.', warn:'Redis tiene memoria máxima configurable. Si se llena, empieza a borrar datos. Configura la política de eviction correctamente (allkeys-lru).' },
  ...
  { name:'Cache-Control headers', type:'Caché de browser', color:'#a8f0c8', def:'Headers HTTP que le dicen al browser (y a los proxies/CDNs) durante cuánto tiempo puede guardar en caché un recurso sin volver a pedirlo al servidor. Cache-Control: max-age=31536000 = caché 1 año.', example:'Nginx: location ~* \\.(js|css)$ { add_header Cache-Control "public, max-age=31536000, immutable"; }', warn:'' },
];

const DOCKER_CONCEPTS = [
  { name:'Contenedor (Container)', type:'Docker fundamental', color:'#4dd9e0', def:'Unidad de software empaquetada con el código, runtime y dependencias. Aislado del sistema host pero más ligero que una VM. Cada servicio (Nginx, Express, Redis) corre en su propio contenedor.', example:'docker run -d nginx → lanza un contenedor Nginx aislado. El host no necesita tener Nginx instalado.', warn:'' },
  { name:'Docker Compose', type:'Orquestación local', color:'#4dd9e0', def:'Herramienta para definir y arrancar múltiples contenedores con un solo archivo (docker-compose.yml). Define servicios, redes internas, volúmenes y variables de entorno. Ideal para desarrollo y proyectos pequeños.', example:'docker-compose up -d → arranca Nginx + Express + Redis todos juntos con su red interna configurada.', warn:'' },
  { name:'Imagen (Image)', type:'Docker fundamental', color:'#4dd9e0', def:'Plantilla inmutable de la que se crean los contenedores. Se construye desde un Dockerfile. Una imagen de Node.js incluye el sistema base, Node, npm y tu código. El contenedor es una instancia en ejecución de una imagen.', example:'FROM node:20-alpine → COPY . . → RUN npm install → CMD ["node","index.js"] → este Dockerfile define la imagen.', warn:'' },
  { name:'Red interna Docker', type:'Seguridad de red', color:'#4dd9e0', def:'Docker Compose crea una red privada entre los contenedores. Solo Nginx es accesible desde fuera. El contenedor Express solo es alcanzable desde dentro de esa red — jamás desde internet directamente.', example:'Nginx hace proxy_pass http://express:3000. "express" es el nombre del servicio — resuelve solo dentro de la red Docker.', warn:'' },
  { name:'Nginx como Reverse Proxy', type:'Servidor web / Proxy', color:'#4dd9e0', def:'Nginx recibe todas las peticiones públicas (HTTPS). Las que son archivos estáticos (/, /assets) las sirve directamente desde disco. Las que empiezan por /api/* las reenvía (proxy_pass) al contenedor Express interno.', example:'location /api/ { proxy_pass http://express:3000; } location / { root /usr/share/nginx/html; try_files $uri $uri/ /index.html; }', warn:'' },
  ...
  { name:'Volumen (Volume)', type:'Persistencia de datos', color:'#4dd9e0', def:'Mecanismo para persistir datos generados por un contenedor. Sin volumen, los datos desaparecen cuando el contenedor se reinicia. Las bases de datos siempre deben usar volúmenes.', example:'volumes: - ./data/mysql:/var/lib/mysql → los datos de MySQL persisten en tu host aunque el contenedor se destruya y recree.', warn:'' },
];
...
const DK_VOLUMES = [
  { name:'Volumen gestionado', sub:'docker volume', color:'#a8f0c8', desc:'Docker gestiona el almacenamiento en /var/lib/docker/volumes. Independiente de la estructura del host. Portátil entre hosts. El modo recomendado para datos de producción.', when:'Bases de datos, datos persistentes de apps, backups.' },
  { name:'Bind mount', sub:'ruta del host', color:'#7eb8f7', desc:'Monta una carpeta específica del host en el contenedor. El contenedor ve y modifica los archivos del host directamente. Ideal para desarrollo: editas en el host y el contenedor lo recoge.', when:'Desarrollo local, archivos de config, logs accesibles desde el host.' },
